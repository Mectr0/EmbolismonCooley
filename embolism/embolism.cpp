...
class VTKData {
public: 
void writeVTKImageData(MultiBlockLattice3D<T, DESCRIPTOR>& lattice,
    IncomprFlowParam<T> const& parameters, plint iter)
{
	T lx = parameters.getlx(); 
	T ly = parameters.getly();
	T lz = parameters.getlz(); 


	vtkSmartPointer<vtkImageData> imageData= 
	 vtkSmartPointer<vtkImageData>::New();

	imageData->SetDimensions.IncomprFlowParam(); 
